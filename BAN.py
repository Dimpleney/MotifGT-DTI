...
class BANLayer(nn.Module):
    ...
    def forward(self, v, q, softmax=True):
        ...
        if softmax:
            ...
            # print("att_maps after softmax:",att_maps.shape)
...
def visualize_attention(att_map, v_num, q_num):
    att_map = att_map.squeeze().detach().cpu().numpy()
    plt.imshow(att_map, cmap='viridis')
    plt.colorbar()
    plt.xlabel('Question')
    plt.ylabel('Image Regions')
    plt.title('Attention Map')
    plt.show()
def test_BANLayer():
    ban = BANLayer(v_dim=16, q_dim=16, h_dim=32, h_out=2, act='ReLU', dropout=0.2, k=3)

    v = torch.rand(1, 10, 16)  # batch_size=8, v_num=10, v_dim=16
    q = torch.rand(1, 20, 16)  # batch_size=8, q_num=20, q_dim=16

    output, att_map = ban(v, q, softmax=True)

    # print("Output size: ", output.size())
    # print("Att map size: ", att_map.size())
    visualize_attention(att_map[:, 0, :, :], v.size(1), q.size(1))
