...
def smiles_to_graph(smiles, max_block, max_sr, min_frag_atoms,explicit_H=False, use_chirality=True):
    # print(smiles)
    ...
    for idx,fr in enumerate(frags):
        g = dgl.DGLGraph()
        f_mol = Chem.MolFromSmiles(fr)
        num_atoms = f_mol.GetNumAtoms()
        # print(num_atoms)
        # add nodes
        g.add_nodes(num_atoms)
        atom_feats = np.array([atom_features(a, explicit_H=explicit_H) for a in f_mol.GetAtoms()])
        if use_chirality:

            chiralcenters = Chem.FindMolChiralCenters(f_mol, force=True, includeUnassigned=True,
        ...
        if num_bonds>0:
            ...
            g.edata["e"] = torch.tensor(np.array(bond_feats_all),dtype=torch.float32)
            g = dgl.add_self_loop(g)
            g = laplacian_positional_encoding(g, pos_enc_dim=8)
            graphs.append(g)
        else:
            g = dgl.add_self_loop(g)
            g.edata["e"] = torch.ones(1, 10)
            g = laplacian_positional_encoding(g, pos_enc_dim=8)
            graphs.append(g)
    g = dgl.batch(graphs)
    # g = laplacian_positional_encoding(g, pos_enc_dim=8)
