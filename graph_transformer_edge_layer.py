...
def message_func(edges):
    return {'m': edges.src['V_h']}

def reduce_func(nodes):
    return {'wV': torch.sum(nodes.mailbox['m'], dim=1)}
...
class GraphTransformerLayer(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, num_heads, dropout, layer_norm=True, batch_norm=False, residual=True,
                 use_bias=False):
        super().__init__()
