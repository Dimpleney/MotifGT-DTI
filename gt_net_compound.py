...
"""
    ...
    使用带有边注意力机制的图形变换层和构建com_gt
"""
import dgl
from g_transformer.graph_transformer_edge_layer import GraphTransformerLayer
...
class GraphTransformer(nn.Module):
    ...
    def forward(self, g):
        # input embedding
        g = g.to(self.device)
        h = g.ndata["h"].float().to(self.device)
        h_lap_pos_enc = g.ndata['lap_pos_enc'].to(self.device)
        e = g.edata["e"].float().to(self.device)
